...
function AppraisalContent() {
  ...
  const handleTabChange = (tab: string) => {
    setActiveTab(tab);
    router.replace(`/associate-dean/appraisal?tab=${tab}`, { scroll: false });
  };
  ...
  const userDesignation = (user?.designation as DesignationValue) ?? "Associate Professor";
          ...
          <TabsList className="w-full h-auto bg-muted/60 backdrop-blur rounded-xl p-1 overflow-x-auto flex gap-1 mb-6">
            {tabs.map((tab) => (
              <TabsTrigger
                ...
                value={tab.id}
                className={cn(
                  "flex-1 min-w-[90px] rounded-lg py-2 px-3 text-xs font-medium transition-all",
                  "data-[state=active]:bg-background data-[state=active]:shadow-sm data-[state=active]:text-primary"
                )}
              >
                <span className="block font-semibold">{tab.label}</span>
                <span className="block text-[10px] opacity-70 hidden sm:block">{tab.description}</span>
              </TabsTrigger>
            ))}
          </TabsList>

          <TabsContent value="A">
            <PartAcademicInvolvement department={department} userId={userId} />
          </TabsContent>
          <TabsContent value="B">
            <PartBResearch department={department} userId={userId} />
          </TabsContent>
          <TabsContent value="C">
            <PartCSelfDevelopment department={department} userId={userId} />
          </TabsContent>
          <TabsContent value="D">
            <PartDPortfolio department={department} userId={userId} isAdmin={isAdminFromDesignation} />
          </TabsContent>
          <TabsContent value="E">
            <PartEExtra department={department} userId={userId} />
          </TabsContent>
          <TabsContent value="F">
            <PartFReview department={department} userId={userId} />
          </TabsContent>
        </Tabs>
      </div>
    </div>
...
export default function AssociateDeanAppraisalPage() {
  return (
