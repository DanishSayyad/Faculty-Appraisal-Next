...
function ExternalEvaluateContent({ facultyId }: { facultyId: string }) {
  ...
  const handleSave = async (isSubmit: boolean) => {
    ...
    try {
      if (isSubmit) {
        // TODO: POST /api/${department}/external_interaction_marks/${user.id}/${facultyId}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5">
      <div className="bg-emerald-700 px-6 py-4 text-white">
        <h1 className="text-xl font-bold">External Interaction Evaluation</h1>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        <Button variant="ghost" size="sm" className="text-emerald-600 hover:text-emerald-800 -ml-2" onClick={() => router.push("/external/dashboard")}>
          <ArrowLeft size={15} className="mr-1" /> Back to Dashboard
        </Button>

        <motion.div variants={containerVariants} initial="hidden" animate="visible">
          <motion.div variants={itemVariants}>
            <Card className="border bg-emerald-50 dark:bg-emerald-950/20 border-emerald-100 dark:border-emerald-800">
              <CardContent className="p-6">
                <div className="flex items-center gap-5">
                  <div className="h-16 w-16 rounded-full bg-emerald-100 dark:bg-emerald-900/40 flex items-center justify-center text-emerald-700 shrink-0">
                    <User size={28} />
                  </div>
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      Evaluate this faculty member based on your expert interaction.
                    </p>
                  </div>
                </div>
              </CardContent>
          ...
          <motion.div variants={itemVariants} className="mt-4">
            <Card className="border bg-emerald-50 dark:bg-emerald-950/20 border-emerald-200">
              <CardContent className="p-6">
                <h3 className="text-base font-semibold text-emerald-800 dark:text-emerald-300 mb-2">Total Score</h3>
                <div className="flex items-baseline gap-2">
                  <span className="text-3xl font-bold text-emerald-700 dark:text-emerald-400">{calcTotal()}</span>
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Submit this external interaction evaluation? This cannot be changed.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
export default function ExternalEvaluatePage({ params }: { params: { facultyId: string } }) {
  return (
    <Suspense fallback={<div className="flex items-center justify-center min-h-screen"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary" /></div>}>
      <ExternalEvaluateContent facultyId={params.facultyId} />
    </Suspense>
  );
}
