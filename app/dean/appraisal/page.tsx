...
function AppraisalContent() {
    ...
    const handleTabChange = (tab: string) => {
        setActiveTab(tab);
        router.replace(`/dean/appraisal?tab=${tab}`, { scroll: false });
    };
            ...
            <div className="max-w-7xl mx-auto px-4 py-6">
                <Tabs value={activeTab} onValueChange={handleTabChange}>
                    ...
                    <TabsContent value="A" forceMount className="mt-0 focus-visible:outline-none data-[state=inactive]:hidden">
                        <PartAcademicInvolvement userDesignation={userDesignation} apiBase={API_BASE} department={department} userId={userId} />
                    </TabsContent>
                    <TabsContent value="B" forceMount className="mt-0 focus-visible:outline-none data-[state=inactive]:hidden">
                        <PartBResearch apiBase={API_BASE} department={department} userId={userId} userDesignation={userDesignation} />
                    </TabsContent>
                    <TabsContent value="C" forceMount className="mt-0 focus-visible:outline-none data-[state=inactive]:hidden">
                        <PartCSelfDevelopment apiBase={API_BASE} department={department} userId={userId} userDesignation={userDesignation} />
                    </TabsContent>
                    <TabsContent value="D" forceMount className="mt-0 focus-visible:outline-none data-[state=inactive]:hidden">
                        <PartDPortfolio apiBase={API_BASE} department={department} userId={userId} userDesignation={userDesignation} isAdminFromDesignation={isAdminFromDesignation} />
                    </TabsContent>
...
export default function DeanAppraisalPage() {
    return (
        <Suspense
            fallback={
